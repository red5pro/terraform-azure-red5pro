...
# SM_IP="1.2.3.4"
# R5AS_AUTH_USER="example_user"
# R5AS_AUTH_PASS="example_password"
...
check_sm_url() {
    ...
    # Check HTTPS
    https_response=$(curl --connect-timeout 15 --max-time 30 -s -o /dev/null -w "%{http_code}" --insecure "https://$SM_IP/as/v1/admin/healthz")
    ...
    # Check HTTP
    http_response=$(curl --connect-timeout 15 --max-time 30 -s -o /dev/null -w "%{http_code}" "http://$SM_IP/as/v1/admin/healthz")
